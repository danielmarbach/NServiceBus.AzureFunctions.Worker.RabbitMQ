...
";
        ...
        public void Init()
        {
            ...
            // By referencing NServiceBusTriggerFunctionAttribute here, NServiceBus.AzureFunctions.Worker.RabbitMQ is forced to load and participate in the compilation.
        ...
        static (string output, ImmutableArray<Diagnostic> diagnostics) GetGeneratedOutput(string source, bool suppressGeneratedDiagnosticsErrors = false)
        {
            ...
            references.Add(MetadataReference.CreateFromFile(typeof(RabbitMQTriggerAttribute).Assembly.Location));
            references.Add(MetadataReference.CreateFromFile(typeof(FunctionContext).Assembly.Location));
            references.Add(MetadataReference.CreateFromFile(typeof(BasicDeliverEventArgs).Assembly.Location));
